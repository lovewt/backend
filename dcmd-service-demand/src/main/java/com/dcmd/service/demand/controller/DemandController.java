...
import com.dc.dcit.common.vo.SysUserSelecte;
...
import com.dcmd.service.demand.utils.ResponseData;
import com.dcmd.service.demand.utils.ResultData;
...
public class DemandController {
            ...
            demandList.parallelStream().forEach(vo -> {
               userList.forEach(user->{
                ...
                productLine.parallelStream().forEach(a -> {
                    if (vo.getProductModuleId().split(VersionUtils.COMMA)[0].equals(String.valueOf(a.getId()))){
                        vo.setProductModuleName(a.getValue());
                    }
                });
                product.parallelStream().forEach(b -> {
                    if (vo.getProductModuleId().split(VersionUtils.COMMA)[0].equals(String.valueOf(b.getSoftwareNum()))) {
                        vo.setProductModuleName(vo.getProductModuleName()+VersionUtils.COMMA+b.getSoftwareName());
                    }
                });
            });
        }
    ...
    public ResponseData multipleFilesUpload(MultipartFile file) throws Exception {
